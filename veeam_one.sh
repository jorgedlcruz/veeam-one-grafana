...
#
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards"
veeamONEFoundationUrl=$(curl -X GET $veeamONEURL -H "Authorization: Bearer $veeamBearer" -H  "accept: application/json" 2>&1 -k --silent)
    VBRDashboardID=$(echo "$veeamONEFoundationUrl" | jq --raw-output '.items[] | select(.name | startswith("Veeam Backup and Replication")) | .dashboardId')

veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID"
...
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWBackupInfraID/datasources"
...
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWProtectedVMSID/datasources"
...
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWBackupWindowID/datasources"
...
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWTopJobsID/datasources"
...
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWJobStatusID/datasources"
...
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWTopReposID/datasources"
...
# Veeam ONE About
##
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/about"
veeamONEAboutUrl=$(curl -X GET $veeamONEURL -H "Authorization: Bearer $veeamBearer" -H  "accept: application/json" 2>&1 -k --silent)
    ...
    curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_ONE_about,voneserver=$voneserver,voneversion=$version vone=1"

##
# Veeam Backup & Replication Overview. This part will check The VONE v11 VBR Overview
# Veeam Backup Window
##
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWBackupWindowID/datasources/$VBRWBackupWindowDSID/data?forceRefresh=false"
veeamONEOverviewUrl=$(curl -X POST $veeamONEURL -H "Content-Length: 0" -H "Authorization: Bearer $veeamBearer" -H  "accept: application/json" 2>&1 -k --silent)

    declare -i arraybackupwindow=0
    veeamONEOverviewUrlLength=$(echo "$veeamONEOverviewUrl" | jq -r '.data | length')
    for row in $(echo "$veeamONEOverviewUrl" | jq -r '.data[]'); do
        if [[ "$veeamONEOverviewUrlLength" == "$arraybackupwindow" ]]; then
            break
        fi
        ...
        # echo "veeam_ONE_backupwindow,voneserver=$voneserver windowbackup=$windowbackup,windowreplica=$windowreplica,windownas=$windownas"
        arraybackupwindow=$arraybackupwindow+1
        
        ##Comment the Curl while debugging
        echo "Writing veeam_ONE_backupwindow to InfluxDB"
        curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_ONE_backupwindow,voneserver=$voneserver windowbackup=$windowbackup,windowreplica=$windowreplica,windownas=$windownas"
...
# Protected VMs
##
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWProtectedVMSID/datasources/$VBRWProtectedVMSDSID/data?forceRefresh=false"
veeamONEProtectedVMsUrl=$(curl -X POST $veeamONEURL -H "Content-Length: 0" -H "Authorization: Bearer $veeamBearer" -H  "accept: application/json" 2>&1 -k --silent)
    ...
    curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_ONE_protectedvms,voneserver=$voneserver protectedvms=$protectedvms,backuedupvms=$backuedupvms,replicatedvms=$replicatedvms,unprotectedvms=$unprotectedvms,restorepoints=$restorepoints,fullbackups=$fullbackupsSize,increments=$incrementsbackupsSize,sourcevmsize=$sourceSize,successratio=$successratio"
...
##
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWBackupInfraID/datasources/$VBRWBackupInfraDSID/data?forceRefresh=false"
veeamONEInventoryUrl=$(curl -X POST  $veeamONEURL -H "Content-Length: 0" -H "Authorization: Bearer $veeamBearer" -H  "accept: application/json" 2>&1 -k --silent)

    totalvbr=$(echo "$veeamONEInventoryUrl" | jq --raw-output ".data[0].name" | awk -F"[()]" '{if ($2) {print $2}else{print 0}}')    
    totalproxy=$(echo "$veeamONEInventoryUrl" | jq --raw-output ".data[1].name" | awk -F"[()]" '{if ($2) {print $2}else{print 0}}')    
    totalrepo=$(echo "$veeamONEInventoryUrl" | jq --raw-output ".data[2].name" | awk -F"[()]" '{if ($2) {print $2}else{print 0}}')    
    totalbackupjob=$(echo "$veeamONEInventoryUrl" | jq --raw-output ".data[3].name" | awk -F"[()]" '{if ($2) {print $2}else{print 0}}')    
    totalreplicajob=$(echo "$veeamONEInventoryUrl" | jq --raw-output ".data[4].name" | awk -F"[()]" '{if ($2) {print $2}else{print 0}}')    
    totalbackupcopyjob=$(echo "$veeamONEInventoryUrl" | jq --raw-output ".data[5].name" | awk -F"[()]" '{if ($2) {print $2}else{print 0}}')    
    totalnasjob=$(echo "$veeamONEInventoryUrl" | jq --raw-output ".data[6].name" | awk -F"[()]" '{if ($2) {print $2}else{print 0}}')    
    totalcdpolicy=$(echo "$veeamONEInventoryUrl" | jq --raw-output ".data[7].name" | awk -F"[()]" '{if ($2) {print $2}else{print 0}}')    
...
# Jobs Status
##
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWJobStatusID/datasources/$VBRWJobStatusDSID/data?forceRefresh=false"
veeamONEJobsUrl=$(curl -X POST $veeamONEURL -H "Content-Length: 0" -H "Authorization: Bearer $veeamBearer" -H  "accept: application/json" 2>&1 -k --silent)

veeamONEJobsUrlLength=$(echo "$veeamONEJobsUrl" | jq -r '.data | length')
declare -i arrayjobs=0
for row in $(echo "$veeamONEJobsUrl" | jq -r '.data[]'); do
    if [[ "$veeamONEJobsUrlLength" == "$arrayjobs" ]]; then
        break
    fi
    ...
    curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_ONE_jobs,voneserver=$voneserver jobsuccess=$jobsuccess,jobwarning=$jobwarning,jobfail=$jobfail $jobdate"
...
##
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWTopJobsID/datasources/$VBRWTopJobsDSID/data?forceRefresh=false"
veeamONEJobsDurationUrl=$(curl -X POST $veeamONEURL -H "Content-Length: 0" -H "Authorization: Bearer $veeamBearer" -H  "accept: application/json" 2>&1 -k --silent)

lastUpdateTime=$(echo "$veeamONEJobsDurationUrl" | jq --raw-output ".lastUpdateTimeUtc")
lastupdate=$(date -d "$lastUpdateTime" +"%s")

veeamONEJobsDurationUrlLength=$(echo "$veeamONEJobsDurationUrl" | jq -r '.data | length')    
declare -i arrayjobduration=0
for row in $(echo "$veeamONEJobsDurationUrl" | jq -r '.data[].duration'); do
    if [[ "$veeamONEJobsDurationUrlLength" == "$arrayjobduration" ]]; then
        break
    fi
    ...
    curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_ONE_jobsduration,voneserver=$voneserver,jobname=$jobname,jobstatus=$jobStatus,jobduration=$jobduration trend=$jobcompare $lastupdate"
...
##
veeamONEURL="$veeamONEServer:$veeamONEPort/api/v2.1/dashboards/$VBRDashboardID/widgets/$VBRWTopReposID/datasources/$VBRWTopReposDSID/data?forceRefresh=false"
veeamONERepositoriesUrl=$(curl -X POST $veeamONEURL -H "Content-Length: 0" -H "Authorization: Bearer $veeamBearer" -H  "accept: application/json" 2>&1 -k --silent)

lastUpdateTime=$(echo "$veeamONERepositoriesUrl" | jq --raw-output ".lastUpdateTimeUtc")
lastupdate=$(date -d "$lastUpdateTime" +"%s")

veeamONERepositoriesUrlLength=$(echo "$veeamONERepositoriesUrl" | jq -r '.data | length')    
declare -i arrayrepositories=0
for row in $(echo "$veeamONERepositoriesUrl" | jq -r '.data[]'); do
    if [[ "$veeamONERepositoriesUrlLength" == "$arrayrepositories" ]]; then
        break
    fi
    ...
    curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_ONE_repositories,voneserver=$voneserver,vbrserver=$backupsrvname,repositoryname=$repositoryname repocapacity=$repocapacity,repofreespace=$repofreespace,repodaysleft=$repodaysleft,trend=$repocompare $lastupdate"
